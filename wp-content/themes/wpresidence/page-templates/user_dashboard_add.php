 ...
 * src: page-templates\user_dashboard_submit.php 
 * This file is part of the WpResidence theme and handles the submission and editing
 * of property listings by users through their dashboard.
 *
 * @package WpResidence
 ...
 * - WpResidence theme functions (wpestate_dashboard_header_permissions, wpresidence_get_option, wpestate_check_for_agency, etc.)
 * - WpResidence template parts
 *
 * Usage:
 * This template is used for property submission and editing in the user's dashboard within the WpResidence theme.
 */
...
$property_features  =  wpestate_get_cached_terms('property_features');
$errors=array();
...
if( isset($_POST) && isset($_POST['action'])  && $_POST['action']=='view' ) {
    ...
    $property_fields_raw = define_property_fields_raw();
    ...
    if ( $paid_submission_status!='membership' || ( $paid_submission_status== 'membership' || wpestate_get_current_user_listings($parent_userID) > 0)  ){ // if user can submit
        ...
        if($post_id) {
            ...
            update_post_meta ( $post_id, 'header_type',   ( intval( wpresidence_get_option('wp_estate_header_type_property_page',''))  +1 )    );
            ...
            $arguments=array(
                ...
                'new_listing_title' => get_the_title($post_id),
            );
